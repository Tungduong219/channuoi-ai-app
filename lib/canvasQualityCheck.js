...
export function checkImageQuality(imgElement) {
  ...
  return { 
    ...
    visualFeatures: extractVisualFeatures(data)
  };
}

/**
 * Phân tích màu sắc thực tế (Histogram Color Ratios) trên Canvas
 */
export function extractVisualFeatures(pixelData) {
  let redCount = 0;
  let greenCount = 0;
  let whiteCount = 0;
  let yellowCount = 0;
  ...
  for (let i = 0; i < pixelData.length; i += 4) {
    ...
    // Red / Crimson / Blood / Hemorrhage
    if (r > 130 && r > g * 1.35 && r > b * 1.35) {
      redCount++;
    }
    // Green / Banana leaf droppings
    else if (g > 100 && g > r * 1.15 && g > b * 1.1) {
      greenCount++;
    }
    // White chalky / Milk / Foam
    else if (r > 190 && g > 190 && b > 190) {
      whiteCount++;
    }
    // Yellow sulfur / Histomoniasis
