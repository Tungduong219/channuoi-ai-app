...
export function isGuestFarm(farmId) {
  return !farmId || farmId === DEFAULT_GUEST_FARM_ID || farmId.startsWith('guest_');
}
...
export async function createFarm(farmData) {
  ...
  // Ghi Firestore — dùng writeBatch để đảm bảo ATOMIC:
  // farm document + members/{uid} phải cùng thành công hoặc cùng thất bại.
  // Nếu ghi tách rời: mạng rớt giữa chừng → farm tồn tại nhưng members trống
  // → chính chủ không đọc được farm vì isFarmMember() trả về false.
  if (isCloudEnabled && db && !isGuestFarm(farmId)) {
    try {
      const batch = writeBatch(db);

      // Ghi 1: Farm document
      const farmRef = doc(db, 'farms', farmId);
      batch.set(farmRef, { ...newFarm, serverCreatedAt: serverTimestamp() }, { merge: true });

      // Ghi 2: OWNER member — bắt buộc, ngay trong cùng batch
      if (farmData.userId) {
      ...
      // commit() là 1 giao dịch duy nhất — hoặc cả 2 thành công, hoặc cả 2 thất bại
      await batch.commit();
      emitSyncStatus('synced');
    } catch (err) {
      console.error('[Firestore] createFarm batch error:', err.code, err.message);
...
export async function migrateGuestData(targetFarmId) {
  ...
  // Ghi lên Firestore bằng writeBatch (atomic và nhanh hơn nhiều)
  if (isCloudEnabled && db && !isGuestFarm(targetFarmId)) {
    try {
      const batch = writeBatch(db);
      for (const flock of migratedFlocks) {
        const ref = doc(db, 'farms', targetFarmId, 'flocks', flock.flockId);
        batch.set(ref, { ...flock, migratedAt: serverTimestamp() }, { merge: true });
      }
      for (const log of migratedLogs) {
        const ref = doc(db, 'farms', targetFarmId, 'health_logs', log.logId);
        batch.set(ref, { ...log, migratedAt: serverTimestamp() }, { merge: true });
      }
      await batch.commit();
      emitSyncStatus('synced');
    } catch (err) {
  ...
  return { migratedFlocksCount: migratedFlocks.length, migratedLogsCount: migratedLogs.length };
}
...
export function subscribeFlocks(farmId, callback) {
  ...
  if (isCloudEnabled && db && typeof window !== 'undefined' && !isGuestFarm(farmId)) {
    try {
...
export async function toggleFlockVaccineStatus(farmId, flockId, scheduleId, isCompleted) {
  ...
  if (isCloudEnabled && db && flockId && !isGuestFarm(farmId)) {
...
/* =============================================================================
   6. FEED & FCR (FEED CONVERSION RATIO) CALCULATION
   ============================================================================= */

/**
 * Tính toán FCR chuẩn thú y: FCR = Tổng khối lượng thức ăn (kg) ÷ Tổng khối lượng gà xuất chuồng (kg)
 * Quy đổi bao cám: 1 bao = 25kg (tiêu chuẩn phổ biến nhất tại VN)
 */
export function calculateFlockFCR(farmId, flockId) {
  if (!farmId || !flockId) {
    return {
      fcrValue: null,
      totalFeedKg: 0,
      ...
      displayLabel: 'Đang theo dõi lứa nuôi',
      rating: null
    };
  }

  const logs = readLocalCache(`logs_${farmId}`, []);
  const flockLogs = logs.filter(l => l.flockId === flockId);

  let totalFeedKg = 0;
  let totalSoldWeightKg = 0;
  let hasSoldEvent = false;

  for (const log of flockLogs) {
    const notesLower = (log.notes || '').toLowerCase();
    
    // 1. Tính tổng thức ăn tiêu thụ (kg)
    if (log.category === 'cam' || log.category === 'thuc_an' || notesLower.includes('cám') || notesLower.includes('thức ăn')) {
      const kgMatch = notesLower.match(/(\d+(?:[.,]\d+)?)\s*(?:kg|cân|ký)/);
      const baoMatch = notesLower.match(/(\d+)\s*(?:bao|gói)/);
      
      if (kgMatch) {
        totalFeedKg += parseFloat(kgMatch[1].replace(',', '.'));
      } else if (baoMatch) {
        const customKgMatch = notesLower.match(/bao\s*(\d+)\s*kg/);
        const kgPerBag = customKgMatch ? parseInt(customKgMatch[1], 10) : 25;
        totalFeedKg += parseInt(baoMatch[1], 10) * kgPerBag;
      } else if (log.amount > 0) {
        totalFeedKg += Math.round(log.amount / 14500);
      }
    }

    // 2. Tính tổng khối lượng gà xuất chuồng (kg)
    if (log.logType === 'REVENUE' || log.category === 'ban_ga' || notesLower.includes('xuất chuồng') || notesLower.includes('bán gà')) {
      hasSoldEvent = true;
      const weightMatch = notesLower.match(/(\d+(?:[.,]\d+)?)\s*(?:kg|cân|ký|tấn|tạ)/);
      if (weightMatch) {
        let wt = parseFloat(weightMatch[1].replace(',', '.'));
        if (notesLower.includes('tấn')) wt *= 1000;
        else if (notesLower.includes('tạ')) wt *= 100;
        totalSoldWeightKg += wt;
      }
    }
  }

  // Đàn chưa xuất bán hoặc chưa đủ dữ liệu cân xuất chuồng
  if (!hasSoldEvent || totalSoldWeightKg <= 0) {
    return {
      fcrValue: null,
      totalFeedKg: Math.round(totalFeedKg),
      totalSoldWeightKg: 0,
      status: 'IN_PROGRESS',
      displayLabel: totalFeedKg > 0 ? `Đã ăn ${Math.round(totalFeedKg).toLocaleString('vi-VN')} kg cám` : 'Đang theo dõi lứa nuôi',
      rating: 'IN_PROGRESS'
    };
  }

  // Tính FCR thực tế (Weight / Weight)
  const rawFcr = totalFeedKg / totalSoldWeightKg;
  const fcrValue = Number(rawFcr.toFixed(2));

  // Đánh giá nhãn động theo chuẩn chăn nuôi
  let rating = 'CHUAN';
  let displayLabel = `FCR ${fcrValue} (Đạt chuẩn)`;

  if (fcrValue < 1.7) {
    rating = 'TOT';
    displayLabel = `FCR ${fcrValue} (Rất tốt)`;
  } else if (fcrValue > 2.2) {
    rating = 'CAO';
    displayLabel = `FCR ${fcrValue} (Hao cám)`;
  }

  return {
    fcrValue,
    totalFeedKg: Math.round(totalFeedKg),
    totalSoldWeightKg: Math.round(totalSoldWeightKg),
    status: 'COMPLETED',
    displayLabel,
    rating
  };
